            ...
            <form wire:submit.prevent="{{ $inventoryId ? 'updateInventory' : 'createInventory' }}">
              <div class="row">
                ...
                <div class="col-md-4 mb-3">
                  ...
                  <input type="text" id="barcode" class="form-control" wire:model.live="barcode" placeholder="Scan barcode here">
                ...
                <div class="col-md-4 mb-3">
                  ...
                  <select id="shelf_id" class="form-select" wire:model="shelf_id">
                    ...
                    @foreach($allShelves as $shelf)
                      <option value="{{ $shelf->id }}">Level {{ $shelf->shelf_level }} (Capacity: {{ $shelf->capacity }})</option>
                    @endforeach
                  </select>
                  @error('shelf_id') <small class="text-danger">{{ $message }}</small> @enderror
                </div>
                <!-- Quantity Input -->
                <div class="col-md-4 mb-3">
                ...
                <tbody>
                  @forelse($allInventory as $inv)
                    <tr>
                      ...
                      <td>
                        <button class="btn btn-sm btn-warning me-1" wire:click="editInventory({{ $inv->id }})">Edit</button>
                        <button class="btn btn-sm btn-danger" wire:click="deleteInventory({{ $inv->id }})"
