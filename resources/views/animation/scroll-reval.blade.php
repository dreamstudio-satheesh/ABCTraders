    ...
    <div>
        ...
        <nav>
            <ol class="breadcrumb justify-content-sm-start align-items-center mb-0">
                ...
                <li class="breadcrumb-item f-w-500 active">Scroll Reveal</li>
            </ol>
        </nav>
    </div>
</div>
...
<div class="container-fluid">
    <div class="row">
        <div class="col-sm-12">
            ...
            <div class="card">
                <div class="card-header">
                    <h2>How to Use Scroll Reveal(Customize options according to requirements)</h2>
                    <p class="f-m-light mt-1">Add <code>.reveal</code> class to any tag for Archive animation like this
                    </p>
                </div>
                <div class="card-body">
                    <div><i class="text-muted">// The Below Function is example of how to initlize reveal</i>
                        <div class="line"><span class="text-danger">if (Modernizr.csstransforms3d)</span><span
                                class="text-success">{</span><br> window.sr = ScrollReveal();<br> sr.reveal('.reveal',
                            {<br> duration: 800,<br> delay: 400,<br> reset: true,<br> easing: 'linear',<br> scale: 1<br>
                            });<br><span class="text-success"> }</span></div>
                    </div>
                </div>
            </div>
            <div class="card">
                <div class="card-header">
                    <h2>Scroll Reveal Settings Options</h2>
                </div>
                <div class="card-body">
                    <div class="p-b-15"><i class="text-muted">// 'bottom', 'left', 'top', 'right'</i>
                        <div class="line">origin: <span class="text-danger">'bottom',</span></div>
                    </div>
                    <div class="p-b-15"><i class="text-muted">// Can be any valid CSS distance, e.g. '5rem', '10%',
                            '20vw', etc.</i>
                    ...
                    <div class="p-b-15"><i class="text-muted">// Starting angles in degrees, will transition from
                            these values to 0 in all axes.</i>
                        <div class="line">rotate: { x: <span class="text-danger">0</span>, y: <span
                                class="text-danger">0</span>, z: <span class="text-danger">0</span> },</div>
                    </div>
                    <div class="p-b-15"><i class="text-muted">// Starting opacity value, before transitioning to the
                            computed opacity.</i>
                        <div class="line">opacity: <span class="text-danger">0</span>,</div>
                    </div>
                    <div class="p-b-15"><i class="text-muted">// Starting scale value, will transition from this value
                            to 1</i>
                        <div class="line">scale: <span class="text-danger"> 0.9</span>,</div>
                    </div>
                    <div class="p-b-15"><i class="text-muted">// Accepts any valid CSS easing, e.g. 'ease',
                            'ease-in-out', 'linear', etc.</i>
                        <div class="line">easing: 'cubic-bezier(0.6, 0.2, 0.1, 1)',</div>
                    </div>
                    <div class="p-b-15"><i class="text-muted">// is the default reveal container. You can pass
                            either:<br> // DOM Node, e.g. document.querySelector('.fooContainer')<br> // Selector, e.g.
                            '.fooContainer'</i>
                        <div class="line">easing: 'cubic-bezier(0.6, 0.2, 0.1, 1)',</div>
                    </div>
                    <div class="p-b-15"><i class="text-muted">// true/false to control reveal animations on
                            mobile.</i>
                        <div class="line">mobile: true,</div>
                    </div>
                    <div class="p-b-15"><i class="text-muted">// true: reveals occur every time elements become
                            visible<br> // false: reveals occur once as elements become visible</i>
                        <div class="line">reset: false,</div>
                    </div>
                    <div class="p-b-15"><i class="text-muted">// 'always' — delay for all reveal animations<br> //
                            'once' — delay only the first time reveals occur<br> // 'onload' - delay only for animations
                            triggered by first load</i>
                        <div class="line">useDelay: 'always',</div>
                    </div>
                    <div class="p-b-15"><i class="text-muted">// Change when an element is considered in the viewport.
                            The default value<br> // of 0.20 means 20% of an element must be visible for its reveal to
                            occur.</i>
                        <div class="line">viewFactor: 0.2,</div>
                    </div>
                    <div class="p-b-15"><i class="text-muted">// Pixel values that alter the container boundaries.<br>
                            // e.g. Set `{ top: 48 }`, if you have a 48px tall fixed toolbar.<br> // --<br> // Visual
                            Aid: https://scrollrevealjs.org/assets/viewoffset.png</i>
                        <div class="line">viewOffset: { top: 0, right: 0, bottom: 0, left: 0 },</div>
                    </div>
                    <div class="p-b-15"><i class="text-muted">// Callbacks that fire for each triggered element
                            reveal, and reset.</i>
                        <div class="line">beforeReveal: <span class="text-danger">function</span> (domEl) {},<br>
                            beforeReset: <span class="text-danger">function</span> (domEl) {},</div>
                    </div>
                    <div><i class="text-muted">// Callbacks that fire for each completed element reveal, and reset.</i>
                        <div class="line">afterReveal: <span class="text-danger">function</span> (domEl) {},<br>
                            afterReset: <span class="text-danger">function</span> (domEl) {}</div>
                    </div>
                </div>
            </div>
        </div>
