<div class="col-sm-12">
    ...
    <div class="mb-3">
        ...
        <textarea class="form-control" rows="3" name="description"
            placeholder="Enter Description">{{ isset($blog->description) ? $blog->description : old('description') }}</textarea>
        @error('description')
            <span class="text-danger">
                <strong>{{ $message }}</strong>
            </span>
        ...
        <div class="theme-form">
            <div class="mb-3">
                ...
                <div class="toolbar-box">
                    ...
                    <div id="editor8">{!! isset($blog->content) ? $blog->content : old('content') !!}</div>
                </div>
                <input type="hidden" name="content"
                    value="{{ isset($blog->content) ? $blog->content : old('content') }}" id="quill-editor-area">
                @error('content')
                <span class="text-danger">
                    <strong>{{ $message }}</strong>
                </span>
    ...
    <div>
        <div class="col-form-label">Categories <span> *</span>
            <select class="js-category-placeholder-multiple col-sm-12" multiple="multiple" name="categories[]">
                ...
                <option value="{{ $key }}" {{ (is_array(old('categories')) && in_array($key, old('categories'))) ||
                    (isset($default_categories) && in_array($key, $default_categories)) ? 'selected' : '' }}>
                    {{ $value }}
                </option>
                @endforeach
            </select>
    ...
    <div>
        <div class="col-form-label">Tags
            <select class="js-tag-placeholder-multiple col-sm-12" multiple="multiple" name="tags[]">
                ...
                <option value="{{ $key }}" {{ (is_array(old('tags')) && in_array($key, old('tags'))) ||
                    (isset($default_tags) && in_array($key, $default_tags)) ? 'selected' : '' }}>
                    {{ $value }}
                </option>
                @endforeach
            </select>
    ...
    <div class="mb-3">
        ...
        <textarea class="form-control" name="meta_description"
            placeholder="Enter Meta Description">{{ isset($blog->meta_description) ? $blog->meta_description : old('meta_description') }}</textarea>
    ...
    <div class="mb-3">
        ...
        <select class="form-select" name="status">
            <option value="1" {{ !($blog->status ?? 1) == 0 ? 'selected' : '' }} @if(old('status')=='1' ) selected
                @endif>{{ __('Active') }}</option>
            <option value="0" {{ ($blog->status ?? 1) == 0 ? 'selected' : '' }} @if(old('status')=='0' ) selected
